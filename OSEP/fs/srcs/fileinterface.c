...
int copyfiletolocal(char *source, char *local)
{
		
	int value = search_file(Ctrl, source);
	...
	int lp = lfopen(source, LF_RDWR);
	int op = open(local, O_RDWR|O_CREAT, 0644);
	int count = 0;
	char buf[BUFSIZ];
	
	memset(buf, '\0', BUFSIZ);

	while( (count = lfread(lp, buf, BUFSIZ)) > 0) {
